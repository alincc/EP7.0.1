...
public class PaginationContributionControl extends  AbstractPaginationControl {
	...
	public void createViewPartControl() {
		createActions();
		int beginningOfPagination = getToolBarItemSizeBeforeAddingPagination();
		addPaginationItems();

		Display.getCurrent().asyncExec(() -> {
			TestIdUtil epTestUtil = EPTestUtilFactory.getInstance().getTestIdUtil();
			epTestUtil.setTestIdsToToolBarManagerWithStyleForPagination(toolBarManager, beginningOfPagination,
					TestIdUtil.NO_ID_TO_BE_IMPLEMENTED);
		});
	}

	private int getToolBarItemSizeBeforeAddingPagination() {
		toolBarManager.update(true);
		ToolBarManager tbm = (ToolBarManager) toolBarManager;
		ToolBar control = tbm.getControl();

		return control.getItems().length;
	}
