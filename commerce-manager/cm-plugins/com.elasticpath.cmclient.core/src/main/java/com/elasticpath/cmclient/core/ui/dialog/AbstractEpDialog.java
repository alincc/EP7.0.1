...
public abstract class AbstractEpDialog extends TitleAreaDialog {
	 ...
	 */
	protected Button createButton(final Composite parent, final int buttonId, final LocalizedMessage message, final boolean defaultButton) {
		Button button = super.createButton(parent, buttonId, message.getValue(), defaultButton);
		EPTestUtilFactory.getInstance().getTestIdUtil().setId(button, message.getId());
		return button;
	}
