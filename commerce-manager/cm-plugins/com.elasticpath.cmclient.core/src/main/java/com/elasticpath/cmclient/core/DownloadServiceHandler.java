...
public class DownloadServiceHandler implements ServiceHandler {
    ...
    public void service(final HttpServletRequest request, final HttpServletResponse response)
        ...
        final String fileNameOnly = FilenameUtils.getName(fileName);
        File file = new File(tmpDir + "/", fileNameOnly); //$NON-NLS-1$
        if (!file.exists()) {
        ...
        String contentDisposition = "attachment; filename=\"" + fileNameOnly + "\""; //$NON-NLS-1$
