...
public abstract class AbstractAdvancedSearchRequestJob<T extends Persistable> {
	...
	private class AdvancedSearchJob extends Job {
		...
		protected IStatus run(final IProgressMonitor monitor) {
			...
			try {
				monitor.beginTask(CoreMessages.get().SearchProgress_StatusBarMessage_StartSearch, WORK_UNITS);
				...
				monitor.setTaskName(CoreMessages.get().SearchProgress_StatusBarMessage_ConvertToObjects);
				...
				try {
					...
					result = new Status(IStatus.ERROR, AdvancedSearchPlugin.PLUGIN_ID, IStatus.ERROR, CoreMessages.get().SearchProgress_Error, null);
				} finally {
