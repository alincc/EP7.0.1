...
public class CatalogBrowseView extends AbstractCmClientView implements ChangeSetMemberSelectionProvider {
	...
	private void initializeToolBar(final IToolBarManager toolBarManager) {
		...
		EPTestUtilFactory.getInstance().getTestIdUtil().setTestIdsToToolBarManager(toolBarManager, TestIdUtil.BROWSE_VIEW_TOOLBAR_ID);
	}
	...
	public void createTreeViewer(final Composite parent) {
		...
		EPTestUtilFactory.getInstance().getTestIdUtil().setTestIdsToTreeAndItsItems(treeViewer.getTree(), TestIdUtil.CATALOG_BROWSE_TREE_ID);
