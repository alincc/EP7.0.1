...
public class PromotionUsageReportSection extends AbstractReportSection {
	...
	public void createControl(final FormToolkit toolkit, final Composite parent,
		...
		parentEpComposite.addLabelBold(PromotionUsageMessages.store, null);
		storeCombo = parentEpComposite.addComboBox(state, data);

		parentEpComposite.addLabelBold(PromotionUsageMessages.currency, null);
		...
		parentEpComposite.addLabelBold(PromotionUsageMessages.fromdate, null);
		fromDatePicker = parentEpComposite.addDateTimeComponent(IEpDateTimePicker.STYLE_DATE_AND_TIME, state, data);

		parentEpComposite.addLabelBoldRequired(PromotionUsageMessages.todate, EpState.EDITABLE, data);
		toDatePicker = parentEpComposite.addDateTimeComponent(IEpDateTimePicker.STYLE_DATE_AND_TIME, state, data);

		onlyPromotionsWithCouponCodes = parentEpComposite.addCheckBoxButton(PromotionUsageMessages.onlyWithCoupons, state, data);
		EPCustomThemeUtil.setCustomStyle(onlyPromotionsWithCouponCodes, EPCustomThemeUtil.CustomStyle.BOLD, false);
	...
	public String getReportTitle() {
		return PromotionUsageMessages.reportTitle;
	}
	...
	private String[] convertCurrencies(final List<Currency> currenciesList) {
		...
		result[0] = PromotionUsageMessages.selectCurrency;
	...
	private String[] getStoreNames() {
		if (CollectionUtils.isNotEmpty(availableStores)) {
			...
			names[0] = PromotionUsageMessages.selectAll;
