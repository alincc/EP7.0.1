...
public class ProductTypeDependentExporterImplTest {
		...
		context.checking(new Expectations() {
			{
				...
				will(returnValue(new HashSet<>(Arrays.asList(skuOption))));
	...
	public void testFindDependentObjectsNotFiltered() {
		...
		registry.addUidDependencies(ProductType.class, new HashSet<>(Arrays.asList(productType2Uid, productType3Uid)));
