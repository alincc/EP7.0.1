...
public class RuleAdapter extends AbstractDomainAdapterImpl<Rule, RuleDTO> {
	...
	ConditionsDTO createConditionsDTO(final Rule source) {
		...
		final List<RuleCondition> conditions = new ArrayList<>(source.getConditions());
	...
	/**
	 * Finds <code>RuleCondition</code> for the given type and removes it from the list of conditions.
	 *
	 ...
	 */
	BooleanComponentDTO retrieveConditionOfType(final RuleElementType elementType, final List<RuleCondition> conditions) {
		final Optional<ConditionDTO> conditionDTO = conditions.stream()
			.filter(condition -> elementType.equals(condition.getElementType()))
			.map(this::createConditionDTO)
			.findAny();

		conditions.removeIf(condition -> elementType.equals(condition.getElementType()));
