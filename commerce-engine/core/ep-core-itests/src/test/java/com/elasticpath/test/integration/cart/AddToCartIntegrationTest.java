...
public class AddToCartIntegrationTest extends AbstractCartIntegrationTestParent {
	...
	public void testAddSkuIncreasesQuantityRegardlessOfUpdateCommand() {
		...
		shoppingCart = cartDirectorService.addItemToCart(shoppingCart, dto);
		shoppingCart = cartDirectorService.addItemToCart(shoppingCart, dto);
	...
	public void testPersistCartItemWithData() {
		...
		ShoppingCart updatedCart = cartDirectorService.addItemToCart(shoppingCart, dto);
		
		final ShoppingItem addedCartItem = updatedCart.getCartItem(dto.getSkuCode());
		addedCartItem.setFieldValue(key, value);
		assertEquals(value, updatedCart.getCartItems().get(0).getFieldValue(key));
		shoppingCartService.saveOrUpdate(updatedCart);
