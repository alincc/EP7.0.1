...
public class GroovyConditionEvaluatorImplTest {
	...
	public void setUp() {
		...
		final SimpleTimeoutCache<String, FutureTask<Script>> simpleTimeoutCache = new  SimpleTimeoutCache<String, FutureTask<Script>>() {

			private final Map<String, FutureTask<Script>> cache = new HashMap<>();

			@Override
			public FutureTask<Script> get(final String key) {
				return cache.get(key);
			}

			@Override
			public void put(final String key, final FutureTask<Script> value) {
				cache.put(key, value);
			}
		};

		evaluator.setScriptCache(simpleTimeoutCache);
	}
	...
	public void testExpressionEvaluationTrueIfEmpty() {
		...
		assertTrue(evaluator.evaluate(new HashMap<>(), condition));
	}
