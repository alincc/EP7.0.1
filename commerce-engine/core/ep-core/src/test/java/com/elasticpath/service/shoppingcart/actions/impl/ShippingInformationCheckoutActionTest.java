...
public class ShippingInformationCheckoutActionTest {
	...
	public void initializeMockObjects() {
		...
		checkoutActionContext = new CheckoutActionContextImpl(shoppingCart, null, null, null, false, false, null);
	}
		...
		mockery.checking(new Expectations() { 
			{
				...
				atLeast(2).of(shoppingCart).getGuid();
	...
	public void validateCartWithMissingShippingServiceLevel() {
		...
		final List<ShippingServiceLevel> validLevels = new ArrayList<>();
		...
		mockery.checking(new Expectations() { 
			{
				...
				atLeast(2).of(shippingServiceLevel).getGuid();
