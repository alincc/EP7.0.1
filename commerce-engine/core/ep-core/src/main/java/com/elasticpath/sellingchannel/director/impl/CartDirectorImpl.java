...
public class CartDirectorImpl implements CartDirector {
	...
	protected ProductSku getProductSku(final String currentSkuGuid) {
		final ProductSku sku = getProductSkuLookup().findBySkuCode(currentSkuGuid);
	...
	private void checkPricingResults(final ProductPriceMemento productPriceMemento) {
		for (final Map.Entry<String, Price> entry : productPriceMemento.getStack().entrySet()) {
			if (entry.getValue() == null) {
				String errorMessage = "Null price found for " + entry.getKey();
				throw new ProductNotPurchasableException(
						errorMessage,
						asList(
								new StructuredErrorMessage(
										ShoppingCartMessageIds.PRODUCT_MISSING_PRICE,
										errorMessage,
										ImmutableMap.of("product-id", entry.getKey())
								)
						)
				);
			}
	...
	public ShoppingItem addItemToCart(final ShoppingCart shoppingCart, final ShoppingItemDto dto, final ShoppingItem parentItem) {
		...
		if (isProductPurchasableInStore(shoppingCart.getStore(), sku.getProduct(), isDependent, sku.getSkuCode())) {
	...
	public ShoppingItem updateCartItem(final ShoppingCart shoppingCart, final long itemId, final ShoppingItemDto dto) {
		...
		final ProductSku productSku = getProductSkuLookup().findByGuid(item.getSkuGuid());
		if (!isProductPurchasableInStore(store, productSku.getProduct(), isDependent, productSku.getSkuCode())) {
			String errorMessage = "Product is not purchasable.";
			throw new ProductNotPurchasableException(
					errorMessage,
					asList(
							new StructuredErrorMessage(
									ShoppingCartMessageIds.PRODUCT_NOT_PURCHASABLE,
									errorMessage,
									ImmutableMap.of("sku-code", productSku.getSkuCode())
							)
					)
			);
		}
	...
	public void removeAnyNonPurchasableItems(final List<ShoppingItem> items, final ShoppingCart cart) {
		...
		items.removeIf(item -> {
			final ProductSku sku = getProductSkuLookup().findByGuid(item.getSkuGuid());
			if (isProductPurchasableInStore(cart.getStore(), sku.getProduct(), false, sku.getSkuCode())
				&& item.hasPrice()) {
				return false;
			}
			cart.getNotPurchasableCartItemSkus().add(sku.getSkuCode());
			return true;
		});
	}
	 ...
	 * @param skuCode The code of the sku that is attempting to be purchased.
	 * @return true if purchasable
	 */
	public boolean isProductPurchasableInStore(final Store store, final Product product, final boolean isDependent, final String skuCode) {
		final boolean loadProductAssociations = false;

		final StoreProduct storeProduct = storeProductService.getProductForStore(product.getUidPk(), store, loadProductAssociations);
		return storeProduct.isSkuPurchasable(skuCode) && (isDependent || !storeProduct.isNotSoldSeparately());
	}
