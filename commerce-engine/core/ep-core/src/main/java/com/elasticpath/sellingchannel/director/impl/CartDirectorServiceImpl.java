...
public class CartDirectorServiceImpl implements CartDirectorService {
	...
	@Override
	public ShoppingCart addItemToCart(final ShoppingCart shoppingCart, final ShoppingItemDto dto) {
		validateAddAndUpdateCartItem(dto);

		getCartDirector().addItemToCart(shoppingCart, dto);
	...
	@Override
	public ShoppingCart removeItemsFromCart(final ShoppingCart shoppingCart, final Long... doomedItemIds) {
		for (final long doomedItemId : doomedItemIds) {
			shoppingCart.removeCartItem(doomedItemId);
		}
	...
	@Override
	public ShoppingCart moveItemFromWishListToCart(final ShoppingCart shoppingCart, final ShoppingItemDto dto, final String wishlistLineItemGuid) {

		if (!getCartDirector().isSkuAllowedAddToCart(dto.getSkuCode(), shoppingCart)) {
			String errorMessage = "product is not allowed to be added into cart";
			throw new ProductUnavailableException(
					errorMessage,
					asList(
							new StructuredErrorMessage(
									ShoppingCartMessageIds.PRODUCT_NOT_AVAILABLE,
									errorMessage,
									ImmutableMap.of("sku-code", dto.getSkuCode())
							)
					)
			);
		}
	...
	@Override
	public ShoppingItem moveItemFromCartToWishList(final ShoppingCart shoppingCart, final String cartLineItemGuid) {

		ShoppingItem cartItem = shoppingCart.getShoppingItemByGuid(cartLineItemGuid);

		WishList wishList = getWishListService().findOrCreateWishListByShopper(shoppingCart.getShopper());
		ShoppingItem shoppingItem = wishListService.addItem(wishList, cartItem);
		getWishListService().save(wishList);

		removeItemsFromCart(shoppingCart, cartItem.getUidPk());
