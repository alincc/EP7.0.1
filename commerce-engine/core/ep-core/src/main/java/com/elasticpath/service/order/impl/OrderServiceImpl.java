		...
		try {
			...
			if (captureFunds) {
				...
				if (paymentResult != null && paymentResult.getResultCode() != PaymentResult.CODE_OK) {
					String errorMessage = "Can not complete shipment. Payments have failed.";
					throw new CompleteShipmentFailedException(
							errorMessage,
							asList(
									new StructuredErrorMessage(
											OrderMessageIds.PAYMENT_FAILED,
											errorMessage,
											ImmutableMap.of("shipping-number", shipmentNumber == null ? "undefined" : shipmentNumber)
									)
							),
							paymentResult.getCause()
					);
				}
			}
			...
			String shipmentFailedMessage = "Complete shipment failed during release shipment phase.";
			...
			throw new CompleteShipmentFailedException(
					...
					asList(
							new StructuredErrorMessage(
									OrderMessageIds.COMPLETE_SHIPMENT_FAILED,
									shipmentFailedMessage,
									ImmutableMap.of("shipping-number", shipmentNumber == null ? "undefined" : shipmentNumber)
							)
					),
