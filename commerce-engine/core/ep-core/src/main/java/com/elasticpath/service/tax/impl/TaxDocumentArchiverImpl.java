...
public class TaxDocumentArchiverImpl implements TaxDocumentArchiver {
	...
	public void archive(final TaxDocument document, 
		...
		for (TaxJournalRecord taxJournalRecord : getTaxDocumentService().buildTaxJournalRecords(document, taxOperationContext)) {
			getTaxDocumentService().add(taxJournalRecord);
		}
