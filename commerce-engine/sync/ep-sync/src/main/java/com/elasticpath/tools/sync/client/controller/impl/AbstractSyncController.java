...
public abstract class AbstractSyncController implements SyncToolController {
	...
	private SystemConfig createTargetConfig() {
	...
	private SystemConfig createSourceConfig() {
		return new SystemConfig() {
			private BeanFactory beanFactory;
			private ContextInitializer initializer;
	...
	/**
	 * Gets the source {@link SystemConfig} currently in use, creating one (by calling {@link #createSourceConfig()} if no config is currently in use.
	 *
	...
	protected SystemConfig getSourceConfig() {
		if (this.sourceConfig == null) {
			this.sourceConfig = createSourceConfig();
		}
	...
	/**
	 * Gets the target {@link SystemConfig} currently in use, creating one (by calling {@link #createTargetConfig()} if no config is currently in use.
	 *
	...
	protected SystemConfig getTargetConfig() {
		if (this.targetConfig == null) {
			this.targetConfig = createTargetConfig();
		}
