...
public final class ReadDefaultPaymentMethodChoiceCommandImplTest {
	...
	private static final String ENCODED_DEFAULT_PAYMENT_ID = Base32Util.encode(DEFAULT_PAYMENT_ID);
	...
	public void testDefaultPaymentMethodLookup() {
		...
		assertEquals("this should return the expected self URI.",
				URIUtil.format(PAYMENT_METHOD_URI, Selector.URI_PART, ORDER_URI),
				ResourceStateUtil.getSelfUri(result.getData()));
	}
		...
		context.checking(new Expectations() {
			{
				...
				oneOf(paymentMethodUriBuilder).setPaymentMethodId(ENCODED_DEFAULT_PAYMENT_ID);
				will(returnValue(paymentMethodUriBuilder));
	...
	private ReadDefaultPaymentMethodChoiceCommand createReadDefaultPaymentMethodCommand() {
		ReadDefaultPaymentMethodChoiceCommandImpl readDefaultPaymentMethodCommand =
				new ReadDefaultPaymentMethodChoiceCommandImpl(defaultPaymentMethodLookup, paymentMethodUriBuilder,
