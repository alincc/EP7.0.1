...
public final class AddPaymentMethodLinkToOrderStrategyTest {
	...
	@Before
	public void setUp() {
		addPaymentMethodLinkToOrderStrategy = new AddPaymentMethodLinkToOrderStrategy(mockPaymentMethodLookup, paymentMethodInfoUriBuilderFactory);
	...
	public void testLinkToPaymentMethodsForOrderWithPaymentRequiredAndInfoNeeded() {
		...
		ResourceLink needInfoLink = ResourceLinkFactory.createNoRev(PAYMENT_METHOD_INFO_URI, ControlsMediaTypes.INFO.id(), NeedInfoRels.NEEDINFO);

		assertThat("There should be two links created.", createdLinks, Matchers.hasSize(2));
		assertThat(THE_CREATED_LINKS_SHOULD_BE_THE_SAME_AS_EXPECTED, createdLinks, hasItems(createTestPaymentMethodInfoLink(), needInfoLink));
	}
