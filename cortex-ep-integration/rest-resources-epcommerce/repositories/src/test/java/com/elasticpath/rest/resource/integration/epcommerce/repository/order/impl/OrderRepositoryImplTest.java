...
public class OrderRepositoryImplTest {
	...
	public void testGetOrdersForProfileWhenOneFound() {
		...
		ExecutionResult<Collection<String>> result = orderRepository.findOrderIdsByCustomerGuid(STORE_CODE, USER_GUID);
	...
	public void testGetOrdersForProfileWhenNoOrdersFound() {
		...
		shouldGetBean(ContextIdNames.CUSTOMER_SEARCH_CRITERIA, new CustomerSearchCriteria());
		shouldGetBean(ContextIdNames.ORDER_SEARCH_CRITERIA, orderSearchCriteria);
		shouldFindOrderNumbersBySearchCriteria(orderSearchCriteria, Collections.<String>emptyList());

		ExecutionResult<Collection<String>> result = orderRepository.findOrderIdsByCustomerGuid(STORE_CODE, USER_GUID);

		assertTrue("The operation should have succeeded", result.isSuccessful());
		assertTrue("The result set should be empty", result.getData().isEmpty());
	}
