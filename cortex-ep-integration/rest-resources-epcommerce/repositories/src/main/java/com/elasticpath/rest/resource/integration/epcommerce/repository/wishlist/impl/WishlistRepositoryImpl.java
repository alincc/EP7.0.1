...
public class WishlistRepositoryImpl implements WishlistRepository {
	...
	public Observable<String> getWishlistIds(final String customerGuid, final String storeCode) {
		...
		return reactiveAdapter.fromRepositoryAsSingle(() -> customerSessionRepository.findCustomerSessionByGuid(customerGuid))
	...
	public Single<ShoppingItem> addItemToWishlist(final String wishlistId, final String storecode, final String sku) {
		return getWishlist(wishlistId)
				.flatMap(wishList -> getStore(storecode)
	 ...
	 * @return the store
	 */
	protected Single<Store> getStore(final String storecode) {
		return reactiveAdapter.fromRepositoryAsSingle(() -> storeRepository.findStore(storecode));
	}
	 ...
	 */
	protected Single<ShoppingItem> addItemToWishlist(final String sku, final WishList wishList, final Store store) {
	...
	@Override
	@CacheResult
	public Observable<WishList> findWishlistsContainingItem(final String itemId) {
		return reactiveAdapter.fromRepositoryAsSingle(customerSessionRepository::findOrCreateCustomerSession)
				.flatMap(customerSession -> getWishlistInternal(customerSession.getShopper()))
				.flatMapObservable(filterWishlist(itemId))
	 ...
	 */
	protected Func1<WishList, Observable<? extends WishList>> filterWishlist(final String itemId) {
		return wishList -> getProductSku(itemId)
				.flatMapObservable(productSku -> getWishListContainingItem(wishList, productSku.getGuid()));
	}
	 ...
	 */
	protected Single<ProductSku> getProductSku(final String itemId) {
		return reactiveAdapter.fromRepositoryAsSingle(() -> itemRepository.getSkuForItemId(itemId));
	}
	...
	public Single<ShoppingItem> getShoppingItem(final WishList wishlist, final String lineItemGuid) {
		return Observable.from(wishlist.getAllItems())
				.takeFirst(shoppingItem -> shoppingItem.getGuid().equals(lineItemGuid))
				.switchIfEmpty(Observable.error(ResourceOperationFailure.notFound(ITEM_NOT_FOUND)))
				.toSingle();
	}

	@Override
	public WishlistLineItemIdentifier getWishlistLineItemIdentifier(final String scope, final String wishlistId, final String itemGuid) {
		return WishlistLineItemIdentifier.builder()
				.withWishlistLineItems(WishlistLineItemsIdentifier.builder()
						.withWishlist(WishlistIdentifier.builder()
								...
								.build())
						.build())
				.withLineItemId(StringIdentifier.of(itemGuid))
				.build();
	}
	 ...
	 */
	protected Observable<WishList> getWishListContainingItem(final WishList wishList, final String skuGuid) {
		return Observable.from(wishList.getAllItems())
				.takeFirst(shoppingItem -> skuGuid.equals(shoppingItem.getSkuGuid()))
				.flatMap(shoppingItem -> Observable.just(wishList))
