...
public class CustomerRepositoryImpl implements CustomerRepository {
	...
	@Override
	@CacheRemove(typesToInvalidate = Customer.class)
	public Customer addAddress(final Customer customer, final CustomerAddress address) {
		return customerService.addOrUpdateAddress(customer, address);
	}

	@Override
	@CacheRemove(typesToInvalidate = Customer.class)
	public Customer updateAddress(final Customer customer, final CustomerAddress address) {
		Customer updatedCustomer = customerService.addOrUpdateAddress(customer, address);

		if (updatedCustomer.getPreferredShippingAddress().getGuid().equals(address.getGuid())) {
			final Collection<String> cartOrderGuidsByCustomer = Assign.ifSuccessful(cartOrderRepositoryProvider.get()
					.findCartOrderGuidsByCustomer(customer.getStoreCode(), customer.getGuid()));

			final String firstCartOrderGuid = cartOrderGuidsByCustomer.iterator().next();

			cartOrderRepositoryProvider.get().updateShippingAddressOnCartOrder(address.getGuid(), firstCartOrderGuid, updatedCustomer.getStoreCode());
		}
