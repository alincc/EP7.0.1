...
public class WishlistLineItemEntityRepositoryImpl<E extends WishlistLineItemEntity, I extends WishlistLineItemIdentifier>
	...
	@Override
	public Single<WishlistLineItemIdentifier> create(final WishlistLineItemEntity entity, final IdentifierPart<String> scope) {
		...
		return wishlistRepository.getDefaultWishlistId(scope.getValue())
				.flatMap(wishlistId -> wishlistRepository.addItemToWishlist(wishlistId, scope.getValue(), sku)
						.map(shoppingItem -> wishlistRepository.getWishlistLineItemIdentifier(scope.getValue(), wishlistId, shoppingItem
								.getGuid())));
	}
	 ...
	 * @return function
	 */
	protected Func1<WishList, Single<WishlistLineItemEntity>> toWishlistLineItemEntity(final String lineItemGuid) {
		return wishList -> wishlistRepository.getShoppingItem(wishList, lineItemGuid)
				.flatMap(shoppingItem -> wishlistRepository.getProductSku(wishList, lineItemGuid)
						.flatMap(productSku -> getItemIdForSku(productSku)
	 ...
	 * @return itemId
	 */
	protected Single<String> getItemIdForSku(final ProductSku productSku) {
	...
	public Completable delete(final WishlistLineItemIdentifier lineItemIdentifier) {
		...
		String wishlistId = wishlistIdentifier.getWishlistId().getValue().toString();
		return wishlistRepository.removeItemFromWishlist(wishlistId, lineItemIdentifier.getLineItemId().getValue().toString());
	}
