...
public class CreateRefundWizard extends AbstractWizard {
	...
	public void enterRefundAmount(final String currencyCode, final String refundAmount) {
		clearAndType(getWaitDriver().waitForElementToBeVisible(By.cssSelector(String.format(REFUND_AMOUNT_INPUT_CSS, currencyCode))),
				refundAmount);
	...
	public void enterRefundNote(final String refundNote) {
		clearAndType(getDriver().findElement(By.cssSelector(REFUND_NOTE_INPUT_CSS)), refundNote);
	}
