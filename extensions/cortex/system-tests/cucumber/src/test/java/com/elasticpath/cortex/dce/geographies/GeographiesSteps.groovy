...
Given(~'^there is a list of supported (?:countries|countries and regions) for scope (.+)$') { def storeScope ->
	...
	client.authAsAPublicUserOnScope(storeScope)
			.stopIfFailure()

}

Given(~'^(.+) is (?:a|an) (.+) scope$') { String storeScope, String language ->
	client.authAsAPublicUserOnScope(storeScope)
			.stopIfFailure()
}
...
When(~'^the country Canada and the region British Columbia is selected$') { ->
	client.GET("/geographies/${DEFAULT_SCOPE}/countries")
			.findElement {
		country ->
...
When(~'^I request region (.+)') { def regionId ->
	...
	client.GET("/geographies/${DEFAULT_SCOPE}/countries/${encode(CanadaCountryCode)}/regions/${encode(regionId)}")
			.stopIfFailure()
}
When(~'^I request a region with an undecodable id') { ->
	def CanadaCountryCode = "CA";
	def undecodableId = "2=";
	client.GET("/geographies/${DEFAULT_SCOPE}/countries/${encode(CanadaCountryCode)}/regions/${undecodableId}")
			.stopIfFailure()
}
...
Then(~'^I can obtain the country and region code to create an address$') { ->
	client.GET("/")
			...
			.addresses()
			.addressform()
			.createaddressaction(address: ["country-name": countryCode, "locality": "Vancouver", "postal-code": "V7V7V7", "region": regionCode, "street-address": "1234 Hello World"], name: ["family-name": "itest created", "given-name": "address"])
