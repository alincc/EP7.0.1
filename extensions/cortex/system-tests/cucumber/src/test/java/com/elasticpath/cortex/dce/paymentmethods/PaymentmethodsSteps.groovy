...
//    Mobee default test user has tokens as payment method by default
	client.authRegisteredUserByName(DEFAULT_SCOPE, MOBEE_DEFAULT_TEST_USER)
}
...
Given(~'^a purchase was made with payment token$') { ->
	...
	client.search(PURCHASEABLE_NON_SHIPPABLE_ITEM)
			.addToCart(1)
			.stopIfFailure()
...
Given(~'^a free product was purchased without payment$') { ->
	...
	client.search(ZERO_DOLLAR_ITEM)
			.addToCart(1)
			.stopIfFailure()
	client.submitPurchase()
			.stopIfFailure()
...
Given(~'^I authenticate as a shopper with saved payment methods X Y and Z and payment method X is chosen on their order$') { ->
	...
	client.addTestItemToCart(TOKEN_STORE_TEST_CATEGORY, TOKEN_STORE_PURCHASEABLE_ITEM)
...
Then(~'^the paymentmeans is a paymenttoken type$') { ->
	...
	assertThat(client.body.self.type)
			...
			.isEqualTo("elasticpath.purchases.payment-means-payment-token")
}
...
Then(~'^the paymentmeans is a credit card type$') { ->
	...
	assertThat(client.body.self.type)
			.as("The paymentmeans is not as expected")
			.isEqualTo("elasticpath.purchases.purchase.paymentmeans")
}
...
private registerNewShopperAndAuthenticate() {
	...
	client.registerShopper(TOKEN_SCOPE, FAMILY_NAME, GIVEN_NAME, PASSWORD, USERNAME)
