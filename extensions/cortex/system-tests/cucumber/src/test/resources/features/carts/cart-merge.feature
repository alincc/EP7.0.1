...
Feature: Cart Merge
  ...
  Scenario Outline: Public user's cart is merged to a newly registered user's cart
	Given I have item with code <ITEMCODE> in my cart
	When I register and transition to a new user
  ...
  Scenario Outline: Public user's cart's configurable item is merged to a newly registered user's  cart
  ...
  Scenario Outline: Merge Gift Certificate from public user's cart to a newly registered user's cart
...
#	Gift certificates are considered unique even with identical configuration hence the quantities are added
  Scenario Outline: Identical Gift Certificates are merged into single lineitem in registered user's cart
	Given a registered customer <EMAIL_ID> with the following configured item in their cart
	  | itemcode                       | <GIFTCARD>        |
	  | itemqty                        | 1                 |
	  | giftCertificate.message        | <MESSAGE>         |
	  | giftCertificate.recipientEmail | <RECIPIENT_EMAIL> |
	...
	And I have a <GIFTCARD> in the cart with quantity 1 and configurable fields:
	  ...
	  | giftCertificate.recipientName  | <RECIPIENT_NAME>  |
	  | giftCertificate.senderName     | <SENDER_NAME>     |
	When I transition to the registered user with email ID <EMAIL_ID>
	Then the number of cart lineitems is 1
 ...
 #	Configurable items are considered identical with identical configuration hence the public user's quantity is used
  Scenario Outline: Identical configurable items are merged into single lineitem in registered user's cart and later quantity takes precedence
	Given a registered customer <EMAIL_ID> with the following configured item in their cart
	  | itemcode                        | <ITEMCODE>      |
	  | itemqty                         | 1               |
	  | allFieldTypes.boolean           | <BOOLEAN>       |
  ...
  Scenario Outline: Registered shopper's cart items should not merge to another registered shopper's cart
	...
	When I transition to the first registered user
	Then the number of cart lineitems is 1
  ...
  Scenario Outline: Cart merge from public shopper to registered shopper with same item latest quantity takes precedence
	...
	When I transition to the newly registered user
	Then the number of cart lineitems is 1
