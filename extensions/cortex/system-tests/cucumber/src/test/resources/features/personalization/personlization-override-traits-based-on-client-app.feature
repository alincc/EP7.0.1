...
Feature: Test overriding traits from a client application
  ...
  Scenario Outline: Verify the default product price for each gender
    When I authenticate as a registered customer <USERNAME> with the default scope
  ...
  Scenario Outline: Verifying purchase price of customers from different countries
    Given I am logged in as a public user
