...
Feature: Testing CE customer-trait based pricing
  ...
  so that I could display personalised pricing information to the customer
  ...
  Scenario: Discount product price for registered shopper without previous orders
    ...
    When I authenticate as a registered customer itest.default.user@elasticpath.com with the default scope
  ...
  Scenario: Discount product price for newly registered customer
    Given I am logged in as a public user
    And Transformers Registered has an original purchase price equal to $29.99
  ...
  Scenario Outline: Discount product price based on customer age (50 year threshold - customers pre-configured in test data)
    ...
    When I authenticate as a registered customer <CUSTOMER_EMAIL> with the default scope
  ...
  Scenario: Product price for unregistered shopper using First-Time-Buyer Price List
    Given I authenticate as a registered customer itest.default.user@elasticpath.com with the default scope
  ...
  Scenario Outline: Unique product pricing based on different scopes
    Given I am logged into scope <SCOPE> as a public user
