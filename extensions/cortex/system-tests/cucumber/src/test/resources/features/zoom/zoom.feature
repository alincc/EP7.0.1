...
Feature: Zoom
  ...
  Background:
    ...
    And I add item Hugo to the cart with quantity 1
  ...
  Scenario: The same zoom parameter is returned in the response self uri
    When I GET /slots/mobee?zoom=element,element:onevent
    Then the json path self.uri equals /slots/mobee?zoom=element,element:onevent

  Scenario: The return representation self uri should contain a zoom parameter that was not found
    When I GET /slots/mobee?zoom=elephant
    Then the json path self.uri equals /slots/mobee?zoom=elephant
  ...
  Scenario: If the zoom query contains rel that are not found, the return representation self uri should contain the zoom parameter that was not found
    When I GET /slots/mobee?zoom=elephant,element
    Then the json path self.uri equals /slots/mobee?zoom=element,elephant
  ...
  Scenario: POST with valid followlocation and zoom query parameters
    Given I POST to /searches/mobee/keywords/items?zoom=next&followlocation with request body {"keywords":"Hugo"}
    And save the zoom response
    When I POST to /searches/mobee/keywords/items?followlocation with request body {"keywords":"Hugo"}
    And I zoom the current url next
    Then the response is identical to the saved response

  Scenario: POST with zoom specified without followlocation
    When I POST to /searches/mobee/keywords/items?zoom=element with request body {"keywords":"Hugo"}
    Then the HTTP status is OK, created
    And the response is empty

  Scenario: Unsuccessful POST with valid followlocation and zoom query parameters
    When I POST to /searches/mobee/keywords/items?zoom=next&followlocation with request body {"keywords":""}
