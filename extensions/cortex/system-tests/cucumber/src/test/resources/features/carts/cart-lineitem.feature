...
#   Item 2
    ...
    And I add the item to the cart with quantity 1 and configurable fields:
      ...
      | giftCertificate.recipientName  | <RECIPIENT_NAME>  |
      | giftCertificate.senderName     | <SENDER_NAME>     |
    Then the cart lineitem with itemcode <ITEMCODE_2> has quantity 1 and configurable fields as:
      | giftCertificate.message        | <MESSAGE>         |
      | giftCertificate.recipientEmail | <RECIPIENT_EMAIL> |
      | giftCertificate.recipientName  | <RECIPIENT_NAME>  |
      | giftCertificate.senderName     | <SENDER_NAME>     |
...
#   Item 2
  ...
  Scenario Outline: Same configurable items with different field values should in separate cart lineitem
#   Item 1
    When I look up an item with code <ITEMCODE>
    And I go to add to cart form
    And I add the item to the cart with quantity <QTY> and configurable fields:
...
#   Item 2
  ...
  Scenario Outline: Shippable product can be added to cart
    When I look up and add item code <SHIPPABLE_ITEM> to my cart
