...
Feature: Tests order resource
  ...
  Scenario: Order access restricted to owner
    ...
    And I add item Twilight to the cart
    And I retrieve the order
    And save the order uri
    Given I have authenticated as a newly registered shopper
    And attempt to access the other users order
  ...
  Scenario Outline: Free order
    ...
    When I look up and add item code <FREE_ITEM> to my cart
    And I retrieve the order
  ...
  Scenario Outline: Need info links in order and purchaseform resource
    Given I am logged in as a public user
    And I look up and add item code <SHIPPABLE_ITEM> to my cart
    When I retrieve the order
    Then there is a needinfo link to payment-method-info
    And there is a needinfo link to billing-address-info
    And there is a needinfo link to email-info
    And there is a needinfo link to destination-info
    And I retrieve the purchase form
    And there is a needinfo link to payment-method-info
    And there is a needinfo link to billing-address-info
    And there is a needinfo link to email-info
    And there is a needinfo link to destination-info
  ...
  #fill in needinfo's and verify links are removed
    ...
    And I retrieve the order
    Then there is no needinfo link to payment-method-info
    And there is no needinfo link to billing-address-info
    And there is no needinfo link to email-info
    And there is no needinfo link to destination-info
    And I retrieve the purchase form
    And there is no needinfo link to payment-method-info
    And there is no needinfo link to billing-address-info
    And there is no needinfo link to email-info
    And there is no needinfo link to destination-info
  ...
  Scenario Outline: Shipping option need info links
    Given I have authenticated as a newly registered shopper
    And I look up and add item code <SHIPPABLE_ITEM> to my cart
    And I create address with Country GB, Extended-Address County with no shipping options, Locality London, Postal-Code N0N 0N0, Region , Street-Address fake street, Family-Name Test and Given-Name Test
    And I fill in payment methods needinfo
    When I retrieve the order
    Then there is a needinfo link to shipping-option-info
    And I retrieve the purchase form
    And there is a needinfo link to shipping-option-info
    And there are no submitorderaction links
#fill in needinfo's and verify links are removed
    When I add an address with country CA and region BC
    And resolve the shipping-option-info needinfo
    And I retrieve the order
    Then there is no needinfo link to shipping-option-info
    And I retrieve the purchase form
  ...
  Scenario Outline: Verify that deliveries show up based on the type of products in your cart
    Given I have authenticated as a newly registered shopper
    When I look up and add item code <DIGITAL_ITEM> to my cart
    And I retrieve the order
    And I follow links deliveries
    Then there are 0 links of rel element
    And I look up and add item code <PHYSICAL_ITEM> to my cart
    And I retrieve the order
    And I follow links deliveries
  ...
  Scenario: Cannot submit order when needinfo link exists
    ...
    And I add item Samsung Focus to the cart
    And I create address with Country GB, Extended-Address County with no shipping options, Locality London, Postal-Code N0N 0N0, Region , Street-Address fake street, Family-Name Test and Given-Name Test
    And I create an email for my order
    And I fill in payment methods needinfo
    And I retrieve the purchase form
  ...
  Scenario Outline: Can submit order in French scope
    ...
    And I look up and add item code <FRENCH SCOPE SKU> to my cart
