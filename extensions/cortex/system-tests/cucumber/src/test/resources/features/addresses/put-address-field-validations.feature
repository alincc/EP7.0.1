...
Feature: PUT address field validations
	...
	Scenario Outline: PUT address with invalid country values
		...
		And I should see validation error message with error type and debug message
			...
			| field.invalid.size           | country-name size must be between 2 and 2              |
			| field.invalid.country.format | country-name does not exist in list of supported codes |
	...
	Scenario: Can modify an address with optional fields left empty
		...
		And I create address with Country CA, Extended-Address 1234, Locality Vancouver, Postal-Code V5C1N2, Region BC, Street-Address 123 Broadway, Family-Name Customer and Given-Name Public Tester
		...
		And I modify the address with Country US, Extended-Address , Locality Vancouver, Postal-Code 90210, Region WA, Street-Address 123 Broadway, Family-Name Customer and Given-Name Public Tester
		Then the HTTP status is no content
